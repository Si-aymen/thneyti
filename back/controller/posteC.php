...
public function ajouterposte($poste){
	$pdo=config::getConnexion();
	try {
		$query=$pdo->prepare(
			"INSERT INTO poste (id_poste,titre,contenu,date_de_pub) VALUES (:id_poste,:titre,:contenu,:date_de_pub);"
		);
		$query->execute([
...
function modifierp($posteC,$id_poste)
        {
                $sql="UPDATE poste SET id_poste=:id_poste,titre=:titre,contenu=:contenu,date_de_pub=:date_de_pub WHERE id_poste=:id_poste";
        ...
        try{        
              ...
              //  $commentaire=$reservation->getcommentaire();
                $datas = array(':id_poste'=>$id_poste, ':titre'=>$titre,'contenu'=>$contenu,  ':date_de_pub'=>$date_de_pub);
